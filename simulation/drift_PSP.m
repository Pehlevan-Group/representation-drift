...
for i = 1:1e4
        curr_inx = randperm(tot_iter,1);  % randm generate one sample
...
% saveas(f_learnCurve,[saveFolder,filesep,prefix,'.fig'])
...
sel_inx = randperm(tot_iter,num_sel);    % samples indices
...
ensemble_step = 1e4;           
partial_index = randperm(tot_iter,subpopulation);   
...
if strcmp(learnType,'offline')
    for i = 1:tot_iter
        ...
        zetas = randn(output_dim,output_dim);
        Y = pinv(M)*W*X;
        W = (1-2*dt)*W + 2*dt*Y*X'/tot_iter + sqrt(2*dt)*stdW*xis;
        M = (1-dt/tau)*M + dt/tau*Y*Y'/tot_iter +  sqrt(dt/tau)*stdM*zetas;
        
        % readout matrix and error
        Wout = Wout + dt*(labels - Wout*Y)*Y'/tot_iter;
...
elseif strcmp(learnType,'online')
    for i = 1:tot_iter
        ...
        Y = pinv(M)*W*X(:,curr_inx); 
        W = (1-2*dt)*W + 2*dt*Y*X(:,curr_inx)' + sqrt(dt)*stdW*xis;
        M = (1-dt/tau)*M + dt/tau*Y*Y' +  sqrt(dt)*stdM*zetas; 
...
ensem_sel = [2, 4];   % this should depdend on the learning type
