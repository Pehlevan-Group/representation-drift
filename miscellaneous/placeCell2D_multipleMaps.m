% First, reproduce the simulation result in Manaco and Abbott's paper
% other part of the script is modified from "placeCells.m"
% 
% last revised on 10/05/2021
...
param.C = 0.25;
param.alpha = 100/param.Np/param.C; 
param.J = 2e3;
param.lbd = 0.2;
param.learnRate = learnRate;  % learning rate for W and b
param.noise =  noiseStd;   % stanard deivation of noise 

param.W = zeros(param.Np,param.Ng);
refVec = rand(1,param.Ng);
refVec(rand(1,param.Ng)>param.C) = 0;
for i = 1:param.Np
    param.W(i,:) = refVec(randperm(param.Ng));
end
wlen = norm(refVec);  % the length of weight vector of each place cell

gridQuality = 'regular';  % regular or weak
learnType = 'batch';    % bath mode of learning
...
% this part is based on the Monaca and Abbott's paper
...
elseif strcmp(learnType, 'batch')
    for i = 1:tot_iter
        ...
        r = PlaceCellhelper.multipleMapDynBatch(positions, param);  %firing rate of place cell
        param.W = param.W + param.learnRate*r*positions'/BatchSize;
        % normalize the weight
        Wlens = sqrt(sum(param.W.^2,2));
        param.W = param.W./(Wlens*ones(1,param.Ng))*wlen;
