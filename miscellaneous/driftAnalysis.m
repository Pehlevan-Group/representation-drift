...
if strcmp(learnType,'offline')
    ...
    %     M = (1-dt/tau)*M + dt/tau*(Y*Y'/t +  noise*randn(k,k));
        ...
        if mod(i,step)==0
            ...
            % delta learning rate
%             Ysel = temp*X(:,sel_inx);  % readout of selected input
%             Wout(round(i/step)+1,:) = Wout(round(i/step),:) + dt*(labels -...
%                 - Wout(round(i/step),:)*Y)*Y'/t;
...
%             evsOld = evs;  % update the old eigenvectors           
        end
        
        % store the overall perturbation matrices
%         if mod(i,record_step)==0
%            Minv = pinv(M);
%            F = Minv*W;
%            perturbs(:,:,round(i/record_step)) = sqrt(dt)*Minv*(noise1*xis - noise2*zetas*F);
%         end
