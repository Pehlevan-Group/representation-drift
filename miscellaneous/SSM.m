classdef SSM < handle
   ...
   methods
      function obj = SSM(k, d, tau0, Minv0, W0, learning_rate)
          obj.k = k;
          obj.d = d;
          if isempty(W0)   
              obj.W = randn(k,d)/d;
...
%           z = obj.Minv * y;
%           c = step / (1 + step * z' * y);
          obj.outer_Minv  = step*(y*y');
          obj.Minv = (1-step)*obj.Minv + obj.outer_Minv;
          obj.t  =  obj.t + 1;
          
      end
      
     function y = fit_next_noisy(obj,x,noise_level)
          % return the projection into the subspace 
%           y = obj.Minv * (obj.W * x');
          y = obj.Minv \ (obj.W * x');
          %Plasticity, using gradient ascent/descent
          ...
          step = obj.lr(obj.t);          
          obj.outer_W = (2 * step * y) * x;
%           obj.W = (1 - 2 * step*(1+noise_level*randn)) * obj.W + obj.outer_W;
          obj.W = (1 - 2 * step) * obj.W + obj.outer_W + step*noise_level*randn(size(obj.W)); 
          % M <- M + eta(self.t)/tau * (y*y' - M), using SMW
          step = step / obj.tau;
%           obj.Minv = obj.Minv / (1 - step);
%           z = obj.Minv * y;
%           c = step / (1 + step * z' * y);
          obj.outer_Minv  = step*(y*y');
%           obj.Minv = (1-step*(1+noise_level*randn))*obj.Minv + obj.outer_Minv;
          obj.Minv = (1-step)*obj.Minv + obj.outer_Minv + step*noise_level*randn(size(obj.Minv));
          obj.t  =  obj.t + 1;
