% This program demonstrates that drift happens only when the objective
% function is degenerate
% implement Sanger's rule
...
% Desired PCA of selected inputs
Zpca = V(:,1:k)'*X(:,sel_inx);
...
ensem_sel = [1, 3];   % this should depdend on the learning type
...
% %% 
% figure
% plot((1:size(Ytest,2))'*step,Ytest(:,:,13)')
...
for i = 1:k
    ...
    hold on
%     plot(xs,ones(length(xs),1)*Zpca(i,selYInx),'k--')
end
hold off
...
% spectrumData = load('./data/eigenSpectr0820.mat');  % previous data
...
% xlim([2e1,2e4])
% xlabel('$\sum_i^{k}1/\lambda_i^2$','Interpreter','latex','FontSize',labelFontSize)
% ylabel('$D_{\varphi}$','Interpreter','latex','FontSize',labelFontSize)
% set(gca,'LineWidth',1.5,'FontSize',gcaFontSize,'XScale','log','YScale','log',...
%     'YTick',10.^(-5:1:-2))
