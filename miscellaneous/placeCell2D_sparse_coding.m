% using non-negative sparse coding to learn a 2-d place cells based
% on predefined grid cells patterns. Based on the paper lian et al 2020
% Model 1m x 1m 2d environemnt
...
param.W = normalize_matrix(randn(param.Np,param.Ng),1);   % initialize the forward matrix
param.M = param.W*param.W' - eye(param.Np);       % this weight matrix doesn't change

param.lbd1 = 0.2;               % 0.15 for 400 place cells and 5 modes
...
param.step = 10;              % recording time seperation
param.ampThd = 0.2;           % threshold to define if a bump is qualified as RF
...
Z0 = zeros(numIN,param.BatchSize);  % initialize interneurons
Y0 = zeros(param.Np, param.BatchSize); 
...
for i = 1:total_iter
    positions = gdInput(:,randperm(param.ps*param.ps,param.BatchSize));
...
[output, param] = place_cell_stochastic_update_sparse_coding(gdInput,total_iter, param, 2);

allbias = nan(total_iter/sep,1);

pks = output.pks;    % store all the petak positions
pkAmp = output.pkAmp;  % store the peak amplitude
% placeFlag = nan(param.Np,time_points); % determine whether a place field

pkCenterMass = output.pkCenterMass;  % store the center of mass
pkMas = output.pkMas;           % store the average ampltude 
...
figure
plot((1:size(pkAmp,2))*step,mean(pkAmp > param.ampThd,1),'LineWidth',2)
